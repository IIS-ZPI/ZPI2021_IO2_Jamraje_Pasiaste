...
public class JsonParser {
	...
	public List<Currency> getAvailableCurrenciesList() throws IOException {
		...
		String[] temp;
		List<String> tables = List.of("http://api.nbp.pl/api/exchangerates/tables/A?format=json", "http://api.nbp.pl/api/exchangerates/tables/B?format=json");

		for (String table : tables) {
			jsonResponse = getJsonResponseFromURL(table);

			tableName = String.valueOf(jsonResponse.json().readArray().getJsonObject(0).getJsonString("table")).replaceAll("\"", "");

			effectiveDate = String.valueOf(jsonResponse.json().readArray().getJsonObject(0).getJsonString("effectiveDate")).replaceAll("\"", "");

			for (JsonValue array : jsonResponse.json().readArray().getJsonObject(0).getJsonArray("rates")) {
				temp = array.toString().replaceAll("[\\{:,}]", "").split("\"");
